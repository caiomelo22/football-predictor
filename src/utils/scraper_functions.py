...
def scrape_fbref(league, league_id):
    ...
    url = (
        f"https://fbref.com/en/comps/{league_id}/schedule/{league}-Scores-and-Fixtures"
    )
    driver.get(url)
    ...
    for i, r in enumerate(rows):
        ...
        if len(tds) == 12:
            date, _, home_team, home_xg, score, away_xg, away_team, _, _, _, _, _ = [
                t.text for t in tds
            ]
            home_squad_id, away_squad_id = get_teams_squad_id(2, tds)
        elif len(tds) == 13:
            _, date, _, home_team, home_xg, score, away_xg, away_team, _, _, _, _, _ = [
                t.text for t in tds
            ]
...
def scrape_betexplorer(season_games, league, league_country):
    ...
    for i, r in enumerate(rows):
        ...
        try:
            if len(tds) < 6:
                continue
            _, matchup, _, _, _, home_odds, draw_odds, away_odds, date = [
                t.text for t in tds
            ]
    ...
    for i, row in season_games.iterrows():
        ...
        try:
            ...
            same_date_matches = odds_df[
                (odds_df["date"].dt.date == row["date"].date())
                | (minus_one_day.date() == odds_df["date"].dt.date)
                | (plus_one_day.date() == odds_df["date"].dt.date)
            ].reset_index(drop=True)
            if not len(same_date_matches):
                continue
            same_date_matches["matchup_score"] = same_date_matches.apply(
                lambda x: set_fuzz_score(row["home_team"], row["away_team"], x), axis=1
            )
...
def scrape_advanced_stats(
    league_id, season_test, seasons_squad_ids, league_selected_stats
):
    driver = initialize_driver()
    games_stats_dict = {}
        ...
        for stat_type in league_selected_stats.keys():
            ...
            for i, r in enumerate(rows):
                ...
                save_game_stats(
                    ...
                    venue,
                    stats,
                    league_selected_stats[stat_type],
                    games_stats_dict,
                )
