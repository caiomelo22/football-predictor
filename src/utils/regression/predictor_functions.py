...
def get_league_data(league, seasons, season_test, y_column):
    ...
    specific_ha_columns = [
        c
        for c in columns
        if (c.startswith(ha_prefix) and "_opp_" not in c)
        or (not c.startswith(ha_prefix) and "_opp_" in c)
    ]
...
def regression_model_evaluation(X_train, y_train, X_test, y_test, pred_col):
    ...
    df_predictions[f"{pred_col} Difference"] = (
        df_predictions[f"Pred {pred_col}"] - df_predictions[f"Actual {pred_col}"]
    )
