...
def set_numerical_categorical_cols(X):
    ...
    categorical_cols = [
        ...
        if (X[cname].nunique() < 10 and X[cname].dtype in ["object"])
    ]

    # Select numerical columns
    numerical_cols = [cname for cname in X.columns if X[cname].dtype in ["float64"]]
...
def train_kmeans(X_train, features_to_explore):
    ...
    for i in range(len(features_to_explore)):
        ...
        X_train[f"Cluster_{i+1}"] = kmeans.fit_predict(
            scale_dataset(X_train.loc[:, features_to_explore[i : i + 2]], kmeans_scaler)
        )
...
def apply_kmeans(X_test, kmeans_scaler_list, features_kmeans_list):
    for i, fk in enumerate(features_kmeans_list):
        features, kmeans = fk
        X_test[f"Cluster_{i+1}"] = kmeans.predict(
            scale_dataset(
                X_test.loc[:, features], kmeans_scaler_list[i], just_transform=True
            )
        )
...
def create_cluster_features(X_train, X_test, mi_scores):  # First mi scores
    ...
    X_train, kmeans_scaler_list, features_kmeans_list = train_kmeans(
        X_train, features_to_explore
    )
...
def train_pca(X_train, mi_scores, min_mi_score=0.001):
    features_to_explore = [
        mi_scores.index[f] for f in range(len(mi_scores)) if mi_scores[f] > min_mi_score
    ]
    print("Total features to consider when doing the PCA:", len(features_to_explore))
    pca = PCA(random_state=0)
    pca_scaler = MinMaxScaler()
    X_pca = pca.fit_transform(
        scale_dataset(X_train.loc[:, features_to_explore], pca_scaler)
    )
...
def apply_pca(X_test, scaler, pca, features_to_explore):
    X_pca = pca.transform(
        scale_dataset(X_test.loc[:, features_to_explore], scaler, just_transform=True)
    )
...
def apply_pca_datasets(
    X_train, X_test, mi_scores, min_mi_score=0.001
):  # Second mi scores
    X_train, features_to_explore, pca_scaler, pca = train_pca(
        X_train, mi_scores, min_mi_score=min_mi_score
    )
...
def get_models():
    ...
    for model in models_dict.keys():
        if models_dict[model]["voting"] and not isinstance(
            models_dict[model]["estimator"], Sequential
        ):
...
def build_pred_df(
    my_pipeline, X_test, y_test, odds_test, bankroll=400, is_neural_net=False
):
    if is_neural_net:
...
def simulate(
    X_train, y_train, X_test, y_test, odds_test, betting_starts_after_n_games, verbose=1
):
