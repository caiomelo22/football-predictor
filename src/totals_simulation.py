...
def get_outcome(row):
    if row['Actual home_score'] > row['Actual away_score']: return 'H'
    elif row['Actual home_score'] < row['Actual away_score']: return 'A'
    else: return 'D'
...
df_predictions[f'Rounded Pred Score'] = (df_predictions['Pred home_score'] + df_predictions['Pred away_score']).round(0)
df_predictions[f'Score Difference'] = (df_predictions['Pred home_score'] + df_predictions['Pred away_score']) - (df_predictions['Actual home_score'] + df_predictions['Actual away_score'])
df_predictions[f'Score Difference Rounded'] = df_predictions[f'Score Difference'].round(0)
...
def get_difference_stats(df_predictions):
    ...
    pred_higher_than_actual = len(rounded_pred_values[rounded_pred_values > 0])/len(rounded_pred_values)
    print(f'{diff_col} Predicted value higher than actual: {round(pred_higher_than_actual, 2)}')
    print(f'{diff_col} Predicted value lower than actual: {round(1-pred_higher_than_actual, 2)}')
