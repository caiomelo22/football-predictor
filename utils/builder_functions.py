...
def get_games_results(games, scenario):
    ...
    return len(games.loc[games['winner'] == scenario]), len(games.loc[games['winner'] == 'D']), len(games.loc[games['winner'] == loser])
...
def get_stats_mean(games, n_last_games, scenario):
    ...
    filtered_cols = [
        ...
        if any(col.startswith(prefix) for prefix in prefixes) and games[col].dtype in (float, 'int32', 'int64')
    ]
...
def drop_total_games_keys(historical_dict):
    ...
    historical_dict = {key: value for key, value in historical_dict.items() if key not in key_to_drop}
...
def get_historical_stats(games, home_games, away_games, suffix=''):
    ...
    historical_keys = [
        'points_pct', 'win_pct', 'draw_pct', 'loss_pct',
        'home_wins', 'home_draws', 'home_losses',
        'away_wins', 'away_draws', 'away_losses'
    ]
    historical_vals = [points_pct, win_pct, draw_pct, loss_pct, home_wins, home_draws, home_losses, away_wins, away_draws, away_losses]
...
def get_team_previous_games(team, game_date, season, fixtures_df):
    ...
    home_games_cols_renamed = {col: col.replace('home_', 'team_').replace('away_', 'opp_') for col in home_previous_games.columns}
    away_games_cols_renamed = {col: col.replace('away_', 'team_').replace('home_', 'opp_') for col in away_previous_games.columns}
...
def get_team_previous_games_stats(team, season, game_date, scenario, n_last_games, fixtures_df):
    ...
    outcome_pct_dict = {
        f'{prefix}_win_pct': games_dict[f'{prefix}_wins'] / len(previous_season_games_filtered),
        f'{prefix}_draw_pct': games_dict[f'{prefix}_draws'] / len(previous_season_games_filtered),
        f'{prefix}_loss_pct': games_dict[f'{prefix}_losses'] / len(previous_season_games_filtered),
    }
